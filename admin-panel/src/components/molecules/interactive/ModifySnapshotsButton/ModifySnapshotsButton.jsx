        ...
        <>
            {debouncedLength ? (
                <Modal
                    ...
                    size="lg"
                    title={t("modify-snapshots.title", { ns: "modals" })}
                >
                    <ModificationForm
                        snapshots={snapshots}
                        close={close}
                    ...
                    <Stack gap="lg">
                        ...
                        >
                            <Text
                                ...
                                fw={600}
                            >
                                {t("modify-snapshots.title-empty", { ns: "modals" })}
                            </Text>
                            <Text
                                size="sm"
                                style={{ textWrap: "" }}
                            >
                                {t("modify-snapshots.description-empty1", { ns: "modals" })}{" "}
                                <IconCameraPlus
                                    size="20"
                                    style={{ transform: "translate(0, 5px)" }}
                                />
                                {" " + t("modify-snapshots.description-empty2", { ns: "modals" })}
                            </Text>
                        </Stack>
                        <Button
                            onClick={onClose}
...
function ModificationForm({ snapshots, close, initiateSnapshotDataUpdate }) {
    ...
    const validationMethod = (val, data, path) => {
        ...
        const nonDeletedInputs = Object.entries(data)
            .filter(([uuid, _]) => !toBeDeletedRef.current[uuid])
            .map(([_, input]) => input);
    ...
    const deleteSnapshots = async () => {
        const uuids = Object.entries(toBeDeletedRef.current)
            .filter(([_, val]) => val)
            .map(([uuid, _]) => uuid);
                ...
                {deletedLength ? (
                    ...
                    >
                        ...
                        <Text
                            ...
                            fw={500}
                        >
                            {t("modify-snapshots.warning", { ns: "modals", count: deletedLength })}
                        </Text>
                    </Group>
                ) : null}
                <Button
    ...
    return (
        ...
        >
            <ActionIcon
                ...
                aria-label={t("modify-snapshots.delete-snapshots-label", { ns: "modals" })}
            ...
            <TextInput
                ...
                flex="1"
                variant="filled"
                placeholder={t("modify-snapshots.name-input-placeholder", { ns: "modals" })}
                disabled={isDeleted}
                {...inputProps}
            />
            <Group
            ...
            >
                <Group
                    className={`${classes.lineStats} ${isDeleted ? classes.disabled : ""}`}
                    justify="right"
                    gap="xs"
                >
