...
address_ns_rasbus(){
   #BR_RASBR inside the NS_RASBUS namespace
    runuser -u CherryWorker -- sudo ip netns exec "${NS_RASBUS}" ip addr add "10.10.10.2/24" dev "${BR_RASBR}" 
}
address_docker_containers(){
    #VETH ends inside Docker containers
    runuser -u CherryWorker -- sudo ip netns exec "${CONTAINER_API}" ip addr add "10.10.10.3/24" dev "${VETH_API_RASBUS}"
    runuser -u CherryWorker -- sudo ip netns exec "${CONTAINER_GUACD}" ip addr add "10.10.10.4/24" dev "${VETH_GUACD_RASBUS}"
}
